...
def get_loss(pred_map, gt, gt_bin, args):
    ...
    if args.cc:
        ...
        loss += args.cc_coeff * nss(pred_map, gt_bin)
        loss += args.cc_coeff * auc_judd(pred_map, gt_bin)
    if args.l1:
...
def loss_func(pred_map, gt, gt_bin, args):
    loss = torch.FloatTensor([0.0]).cuda()
    criterion = nn.L1Loss()
    ...
    if len(pred_map.size()) == 4:
        ...
        for i in range(pred_map.size(0)):
            loss += get_loss(pred_map[i], gt[i],gt_bin[i], args)

        loss /= pred_map.size(0)
        return loss

    return get_loss(pred_map, gt, gt_bin, args)
